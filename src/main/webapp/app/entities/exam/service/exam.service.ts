...
export class ExamService {
  ...
  protected convertDateFromClient(exam: IExam): IExam {
    return Object.assign({}, exam, {
      ...
      startTime: exam.startTime?.isValid() ? exam.startTime.format(DATE_FORMAT) : undefined,
      endTime: exam.endTime?.isValid() ? exam.endTime.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((exam: IExam) => {
        ...
        exam.startTime = exam.startTime ? dayjs(exam.startTime) : undefined;
        exam.endTime = exam.endTime ? dayjs(exam.endTime) : undefined;
      });
    }
    return res;
  }
